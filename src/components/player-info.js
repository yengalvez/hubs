...
AFRAME.registerComponent("player-info", {
  ...
  init() {
    ...
    if (!this.isLocalPlayerInfo) {
      NAF.utils.getNetworkedEntity(this.el).then(networkedEntity => {
        ...
        if (playerPresence) {
          const meta = playerPresence.metas[0];
          this.permissions = meta.permissions;
          this.displayName = meta.profile.displayName;
          this.isBotRunner = !!(meta.context && meta.context.bot_runner);
          if (this.isBotRunner) {
  ...
  updateFromPresenceMeta(presenceMeta) {
    ...
    this.isBotRunner = !!(presenceMeta.context && presenceMeta.context.bot_runner);

    if (!this.isLocalPlayerInfo && this.isBotRunner) {
      this.el.object3D.visible = false;
      this.el.setAttribute("visible", false);
    }
  },
  ...
  applyProperties(e) {
    if (!this.isLocalPlayerInfo && this.isBotRunner) {
      // The technical bot runner should never be visible as a normal user in the room.
      // Keep it hidden even if later systems attempt to toggle visibility.
      this.el.object3D.visible = false;
      this.el.setAttribute("visible", false);
      return;
    }
